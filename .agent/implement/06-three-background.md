# Phase 6: Three.js Particle Background
...
function ParticleField() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      // Gentle sine wave
      positions[i3 + 1] += Math.sin(time * 0.3 + i * 0.01) * 0.002;
  ...
  // Listen for mouse events on window
...
| Setting | Value | Why |
|---------|-------|-----|
| `dpr` | `[1, 1.5]` | จำกัด pixel ratio สูงสุด 1.5x — ไม่เปลือง GPU |
| `antialias` | `false` | ไม่จำเป็นกับ particles, ลด GPU usage |
| `PARTICLE_COUNT` | `1500` | พอให้สวยแต่ไม่หนัก |
| `AdditiveBlending` | — | ทำให้ particles เรืองแสงซ้อนกัน |
| `depthWrite` | `false` | ป้องกัน z-fighting ระหว่างอนุภาค |
...
└──────────────────────────────────────┘
  ...
  Particles drift slowly, react to mouse
```
