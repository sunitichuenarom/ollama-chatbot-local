# Phase 5: useOpenClaw Hook

## File: `hooks/useOpenClaw.ts`
...
|----------|------|-------------|
| `messages` | `ChatMessage[]` | ข้อความทั้งหมด (รวม system prompt) |
| `isStreaming` | `boolean` | กำลังรอ response จาก AI อยู่หรือไม่ |
| `error` | `string \| null` | Error message ล่าสุด |
| `tokenUsage` | `object` | Token usage info สำหรับแสดง UI |
| `sendMessage(content)` | `function` | ส่งข้อความ — จัดการ sliding window อัตโนมัติ |
| `clearMessages()` | `function` | ล้างแชททั้งหมด (เก็บ system prompt) |
| `stopStreaming()` | `function` | หยุด streaming กลางคัน |
...
```env
# These are exposed to the client (NEXT_PUBLIC_ prefix)
NEXT_PUBLIC_SYSTEM_PROMPT=You are OpenClaw AI, a friendly and helpful assistant...
NEXT_PUBLIC_MAX_INPUT_TOKENS=20000
```
